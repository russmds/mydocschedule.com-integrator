...
class SqlCommand implements ISqlCommand
{
	...
	public function __construct($query = null, $orgId = null)
	{
		$this->_query = $query;
		...
		if (is_null($this->_orgId))
		{
			...
			$this->_orgId = $userSession->OrganizationId;
		}
		if ($this->_orgId > -1)
		{
			$this->AddParameter(new Parameter('@orgId', (int)$this->_orgId));
		}
		*/
	}
...
class CountCommand extends SqlCommand
{
	...
	public function GetQuery()
	{
		if (preg_match('/DISTINCT +u\.\*/i', $this->baseCommand->GetQuery()) > 0)
		{
			return preg_replace('/SELECT.+FROM/imsU', 'SELECT COUNT(DISTINCT u.user_id) as total FROM', $this->baseCommand->GetQuery(), 1);
		}else
		{
