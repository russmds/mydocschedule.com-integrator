...
class EmailService implements IEmailService
{
	...
	public function Send(IEmailMessage $emailMessage)
	{
		...
		{
			Log::Debug('Adding email attachment %s', $emailMessage->GetAttachment());
			$this->phpMailer->AddAttachment($emailMessage->GetAttachment());
		}		

		$sendMail = Configuration::Instance()->GetSectionKey(ConfigSection::MAILER, ConfigKeys::SEND_MAIL_FLAG);
		...
		{
			Log::Debug('Logging %s email to: %s from: %s to db', get_class($emailMessage), $toAddresses->ToString(), $from->Address());
	...
	private function _LogEmail()
	{
		$mailer = serialize($this->phpMailer);
		
		ServiceLocator::GetDatabase()->Execute(new CreateEmailCommand($mailer));
		
		//$daemon->ProcessEmails();
	}
}
